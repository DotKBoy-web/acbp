...
-- === Valid masks for clinic_visit (bit-only rules) ===
...
-- === Decision space for clinic_visit ===
CREATE OR REPLACE VIEW "clinic_visit_decision_space" AS
SELECT m.mask, c.*
FROM "clinic_visit_valid_masks" m
CROSS JOIN "clinic_visit_categories" c
;
...
SELECT 'FORBID(booked when SQL: (modality = ''Virtual'' AND department IN (''Imaging'',''Orthopedics'')))'::text AS rule, (NOT ( ((modality = 'Virtual' AND department IN ('Imaging','Orthopedics'))) AND ((((mask >> 0) & 1)) = 1) ))::boolean AS ok
UNION ALL
...
UNION ALL
SELECT 'FORBID(booked when SQL: (site = ''Annex'' AND department IN (''Cardiology'',''Neurology'')))'::text AS rule, (NOT ( ((site = 'Annex' AND department IN ('Cardiology','Neurology'))) AND ((((mask >> 0) & 1)) = 1) ))::boolean AS ok
UNION ALL
