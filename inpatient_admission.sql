...
-- === Valid masks for inpatient_admission (bit-only rules) ===
...
-- === Decision space for inpatient_admission ===
CREATE OR REPLACE VIEW "inpatient_admission_decision_space" AS
SELECT m.mask, c.*
FROM "inpatient_admission_valid_masks" m
CROSS JOIN "inpatient_admission_categories" c
;
...
RETURNS boolean
LANGUAGE sql IMMUTABLE STRICT AS $$
  SELECT ("acbp_is_valid__inpatient_admission"(mask) AND NOT ( ((admission_type = 'Emergency')) AND ((((mask >> 0) & 1)) = 1) ) AND NOT ( ((ward = 'Medical')) AND ((((mask >> 2) & 1)) = 1) ) AND NOT ( ((ward = 'Surgical')) AND ((((mask >> 2) & 1)) = 1) ) AND NOT ( ((ward = 'StepDown')) AND ((((mask >> 2) & 1)) = 1) ) AND NOT ( ((arrival_source = 'Transfer')) AND ((((mask >> 3) & 1)) = 1) ));
$$;
